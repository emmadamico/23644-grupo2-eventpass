...
export const login = async(req, res) => {
    try {
        ...
        return res.status(200).json({msg: "OK", name: user.firstName, last: user.lastName});
    } catch (error) {
        return res.status(500).json({error: "Server error!"});
    }
};
...
export const updateUserInfo = async(req, res) => {
    const {firstName, lastName, email, newEmail, password, newPassw, repeatPassw, securityAnswers} = req.body;
    try {
        let user = '';
        user = await User.findOne({email});
        
        if (user && !password && !repeatPassw && !newPassw && !securityAnswers){
            await User.updateOne({email}, { $set: {"firstName": firstName, "lastName": lastName, "email": newEmail} })
            return res.status(200).json({msg: "User information updated!"})
        } else if (user && password && !securityAnswers){
            const respuestaPassword = await user.comparePassword(password);
            if(!respuestaPassword){
                return res.status(403).json({errorPassw: "Please, verify your password."});
            };
            if(!newPassw || !repeatPassw){
                return res.status(403).json({errorNewPassw: "Required field"});
            };
            if(newPassw != repeatPassw){
                return res.status(403).json({errorNewPassw: "Passwords do not match"});
            };
